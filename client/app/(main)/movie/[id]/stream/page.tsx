...
import { Button, Box, Paper, Typography, CircularProgress, Container, Chip, Stack } from '@mui/material';
...
export default function StreamingPage() {
    ...
    return (
        <Container maxWidth="lg" sx={{ py: 4 }}>
            ...
            {!dbId && (
                <Paper elevation={3} sx={{ p: 4, textAlign: 'center', bgcolor: '#1a1a1a', color: 'white' }}>
                        ...
                        {loading ? (
                            ...
                            >
                                {movie.torrents?.map((torrent: any, index: number) => (
                                    ...
                                    >
                                        {torrent.quality}
                                        {torrent.type && <span style={{ fontSize: '0.8em', opacity: 0.7, marginLeft: '8px' }}>({torrent.type})</span>}
                                    </Button>
                                ))}
                            </Stack>
                        )}
                    ...
                    <Stack direction="row" spacing={1} justifyContent="center" sx={{ mt: 4 }}>
                        {movie.genres?.map((genre: string) => (
                            <Chip key={genre} label={genre} sx={{ bgcolor: '#333', color: '#fff' }} />
                        ))}
                    </Stack>
                </Paper>
            )}
